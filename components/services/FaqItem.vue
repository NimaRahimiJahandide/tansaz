...
const props = defineProps({
  ...
  // New prop to control open/close state externally
  defaultOpen: {
    type: Boolean,
    default: false
  }
...
// Internal state for this FAQ item
...
// Try to get composable from parent (for backward compatibility)
const injectedComposable = inject('faqComposable', null)

// Use internal state or composable
const finalIsOpen = computed(() => {
  if (injectedComposable && typeof injectedComposable.isActive === 'function') {
    // Use composable if available (home page)
    return injectedComposable.isActive(props.itemId)
  } else {
    // Use internal state (service page or standalone)
    return isOpen.value
  }
})

const containerStyle = computed(() => ({
  height: finalIsOpen.value ? `${contentHeight.value}px` : '0px',
  paddingTop: finalIsOpen.value ? '0px' : '0px',
  paddingBottom: finalIsOpen.value ? '0px' : '0px',
  overflow: finalIsOpen.value ? 'visible' : 'hidden'
}))

const contentClasses = computed(() => ({
  'transform translate-y-0 opacity-100': finalIsOpen.value,
  'transform -translate-y-2 opacity-0': !finalIsOpen.value,
}))

const handleToggle = () => {
  if (injectedComposable && typeof injectedComposable.toggleItem === 'function') {
    // Use composable toggle if available (home page)
    injectedComposable.toggleItem(props.itemId)
  } else {
    // Use internal toggle (service page or standalone)
