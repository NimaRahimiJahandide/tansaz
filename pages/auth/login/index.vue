<template>
      ...
      <div class="flex w-full">
        ...
        <div class="w-full lg:w-3/4 p-8 flex items-center justify-center max-sm:bg-[url(/images/login-pic.png)] bg-left-top bg-cover bg-no-repeat">
          <div class="bg-[#FFFFFFCC] sm:bg-white border border-[#F1F1F1] shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] rounded-[16px] p-8 w-full max-w-[627px]">
            <h2 class="text-center text-[24px] font-bold">ورود کاربر</h2>
            <div class="mb-6">
              <label class="block text-right mb-2 text-[#212B36] font-medium">شماره موبایل</label>
              <input type="tel" class="w-full px-4 py-3 rounded text-[#64748B] placeholder-[#64748B] border bg-white border-[#E2E8F0] text-right"
            ...
            <div class="mb-8">
              <label class="block text-right mb-2 text-[#212B36] font-medium">رمز ورود</label>
              <div class="relative">
                <input :type="showPassword ? 'text' : 'password'"
                  class="w-full px-4 pl-10 py-3 rounded border text-[#64748B] placeholder-[#64748B] bg-white border-[#E2E8F0] text-right"
            ...
            <div class="flex w-full justify-between">
              <button @click="$router.push('/auth/forget-password-otp')" class="text-primary font-medium cursor-pointer rounded-lg transition duration-200">
                فراموشی رمز >
              </button>
              <button @click="login" :disabled="v$.$invalid || v$.$errors.length || isLoading"
                ...
                class="sm:w-[142px] w-[100px] bg-primary text-white sm:py-3 py-1 rounded-lg transition duration-200">
                ورود
              </button>
            </div>
          </div>
  ...
  const login = () => {
    ...
    axios.post('/login', formData).then((response) => {
      ...
      dangerDescription.value = "لطفا اطلاعات صحیح وارد کنید";
      setTimeout(() => {
        dangerTitle.value = "";
      }, 300);
      isLoading.value = false
